...
import type { JobSite, JobType, ScrapeOptions } from "./types/job.js";
...
program
  ...
  .option(
    ...
    (value) => value.split(",").map((s) => s.trim()) as JobSite[],
    config.defaultSites,
  )
  ...
  .option("--country <country>", "Country to use for Indeed search", config.defaultCountry)
...
const scrapeOptions: ScrapeOptions = {
  ...
  job_type: opts.jobType as JobType | undefined,
  hours_old: opts.hoursOld,
  country_indeed: opts.country,
};

console.log(`Scraping jobs for "${scrapeOptions.search_term}"...`);

try {
  const jobs = await scrapeJobs(scrapeOptions);
